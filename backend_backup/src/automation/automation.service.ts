...
import { WipService } from '../cost-engine/wip/wip.service';
  ...
  async generateDailyOeeValuation() {
    ...
    try {
      ...
      for (const machine of machines) {
        // 2. Fetch today's MSDR operations for this machine
        ...
        for (const msdr of msdrs) {
          const cutTime = msdr.runningHours ? Number(msdr.runningHours) : 0;
