...
async function main() {
    ...
    const vendor = await prisma.vendor.upsert({
        ...
        create: {
            ...
            vendorType: client_1.VendorType.MATERIAL_SUPPLIER,
    ...
    for (const def of demoProjects) {
        ...
        const currentStageIdx = stageOrder.indexOf(def.currentStage);
        for (let i = 0; i <= currentStageIdx; i++) {
            const fromStage = i === 0 ? 'CREATED' : stageOrder[i - 1];
            const toStage = stageOrder[i];
            const transitionedAt = new Date(def.createdAt.getTime() + (i / (currentStageIdx || 1)) * (now.getTime() - def.createdAt.getTime()));
        ...
        const rtOpInspect = await prisma.routingOperation.create({
            data: {
                ...
                createdBy: 'SEED',
                updatedBy: 'SEED',
                status: ['CREATED', 'ENGINEERING', 'PROCUREMENT', 'MATERIAL_AVAILABLE', 'PRODUCTION'].includes(project.currentStage)
                    ? 'PENDING'
                    ...
                    if (currentStageIdx >= 5) {
                        ...
                        const inspection = await prisma.inspectionHeader.create({
                            data: {
                                ...
                                status: 'COMPLETED',
                                remarks: project.currentStage === 'INSPECTION'
