...
export const BomConverter: React.FC<BomConverterProps> = ({ projectId, project, materials = [], onSaveBOM }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-12 animate-fade-in flex flex-col h-full min-h-0 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-[var(--space-6)] shrink-0">
        
        {/* Left Upload Card */}
        <div className="lg:col-span-2 glass-panel p-[var(--space-6)] relative overflow-hidden flex flex-col justify-center min-h-[220px]">
          <div className="absolute top-0 right-0 w-80 h-80 bg-[var(--color-brand)]/5 rounded-full blur-[80px] pointer-events-none" />
          
          <div 
            className={`border-2 border-dashed rounded-[var(--radius-xl)] p-[var(--space-8)] flex flex-col items-center justify-center transition-all ${
              dragActive 
                ? 'border-[var(--color-brand)] bg-[var(--color-brand)]/10 shadow-[var(--shadow-glow)]' 
                : 'border-[var(--border-500)] bg-[var(--bg-canvas)] hover:border-[var(--color-brand)]/50 hover:bg-[var(--hover-600)]'
            }`}
            ...
            onDrop={handleDrop}
          >
            <Upload className="w-10 h-10 text-[var(--color-brand)] mb-[var(--space-3)] animate-pulse" />
            <h3 className="text-body font-bold text-[var(--text-primary)] mb-[var(--space-1)]">Drag & Drop Engineering BOM Excel</h3>
            <p className="text-caption text-[var(--text-tertiary)] mb-[var(--space-4)]">Compatible with standard structured .xlsx and .xls formats</p>
            ...
            <label 
              ...
              className="bg-[var(--color-brand)] hover:bg-[var(--color-brand-hover)] px-[var(--space-5)] py-[var(--space-2)] rounded-[var(--radius-md)] text-body font-bold text-[var(--text-white)] shadow-[var(--shadow-elevation)] transition-all cursor-pointer"
            >
              Browse Local File
            </label>
          </div>
        ...
        {/* Right Status Panel */}
        <div className="glass-panel p-[var(--space-6)] flex flex-col justify-between">
          <div>
            <h3 className="text-body font-bold text-[var(--text-primary)] mb-[var(--space-4)] tracking-widest uppercase flex items-center">
              <Sliders className="w-4 h-4 mr-[var(--space-2)] text-[var(--color-info)]" />
              Converter Registry
            </h3>
            
            {file ? (
              <div className="space-y-[var(--space-3)]">
                <div className="flex items-center space-x-[var(--space-3)] p-[var(--space-3)] bg-[var(--bg-surface)] border border-[var(--border-500)] rounded-[var(--radius-lg)]">
                  <FileSpreadsheet className="w-8 h-8 text-[var(--color-success)] shrink-0" />
                  <div className="min-w-0">
                    <p className="text-body font-bold text-[var(--text-primary)] truncate">{file.name}</p>
                    <p className="text-caption text-[var(--text-tertiary)]">Size: {(file.size / 1024).toFixed(1)} KB</p>
                  </div>
                </div>

                <div className="space-y-[var(--space-1-5)] text-body text-[var(--text-secondary)]">
                  <div className="flex justify-between">
                    <span>Parsed Project (Tool No):</span>
                    <span className={`font-semibold font-mono ${parsedMetadata?.projectNumber ? 'text-[var(--color-success)]' : 'text-[var(--color-error)]'}`}>
                      {parsedMetadata?.projectNumber || 'Missing Project Number'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Parsed Customer:</span>
                    <span className="font-semibold text-[var(--text-primary)]">{parsedMetadata?.customer || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>BOM Release Date:</span>
                    <span className="font-semibold font-mono text-[var(--text-primary)]">{parsedMetadata?.releaseDate || 'N/A'}</span>
                  </div>
                </div>
              </div>
            ) : (
              <div className="h-28 flex flex-col items-center justify-center text-[var(--text-tertiary)] text-body italic">
          ...
          {file && (
            <div className="flex space-x-[var(--space-3)] mt-[var(--space-4)] shrink-0">
              <button 
                onClick={handleValidate} 
                className="flex-1 py-[var(--space-2)] rounded-[var(--radius-md)] glass-button text-[var(--text-primary)] border border-[var(--border-500)] text-body font-bold transition-all"
              >
                Validate Data
              </button>
              <button 
                onClick={handleConvert} 
                className="flex-1 py-[var(--space-2)] rounded-[var(--radius-md)] bg-[var(--color-brand)] hover:bg-[var(--color-brand-hover)] shadow-[var(--shadow-elevation)] text-[var(--text-white)] text-body font-bold transition-all"
              >
                Convert Rows
              </button>
            </div>
      ...
      {rows.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[var(--space-4)] shrink-0">
          <div className="glass-panel p-[var(--space-4)] flex items-center justify-between border-l-4 border-[var(--color-info)]">
            <div>
              <p className="text-caption text-[var(--text-tertiary)] uppercase tracking-widest font-bold">Total Processed Rows</p>
              <p className="text-xl font-bold text-[var(--text-primary)] font-mono mt-[var(--space-1)]">{totalItemsCount}</p>
            </div>
            <FileSpreadsheet className="w-8 h-8 text-[var(--color-info)] opacity-40" />
          </div>

          <div className={`glass-panel p-[var(--space-4)] flex items-center justify-between border-l-4 ${invalidRowsCount > 0 ? 'border-[var(--color-error)]' : 'border-[var(--color-success)]'}`}>
            <div>
              <p className="text-caption text-[var(--text-tertiary)] uppercase tracking-widest font-bold">Validation Status</p>
              <p className={`text-xl font-bold font-mono mt-[var(--space-1)] ${invalidRowsCount > 0 ? 'text-[var(--color-error)]' : 'text-[var(--color-success)]'}`}>
                {invalidRowsCount > 0 ? `${invalidRowsCount} Errors` : '✓ All Clean'}
              </p>
            </div>
            {invalidRowsCount > 0 ? (
              <AlertCircle className="w-8 h-8 text-[var(--color-error)] opacity-40" />
            ) : (
              <CheckCircle2 className="w-8 h-8 text-[var(--color-success)] opacity-40" />
            )}
          </div>
        </div>
      )}
      ...
      {rows.length > 0 && (
        <div className="glass-panel p-[var(--space-6)] flex-1 min-h-0 flex flex-col relative overflow-hidden">
          
          {/* Section Toolbar */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center shrink-0 mb-[var(--space-4)] gap-[var(--space-3)]">
            <div>
              <h3 className="text-body font-bold text-[var(--text-primary)] flex items-center tracking-widest uppercase">
                <Eye className="w-4 h-4 mr-[var(--space-2)] text-[var(--color-info)]" />
                Purchase order mapping preview
              </h3>
              <p className="text-caption text-[var(--text-tertiary)] mt-[var(--space-1)] uppercase tracking-wider">Configure, resolve errors, and double-check before generation</p>
            </div>

            {/* Filter buttons */}
            <div className="flex items-center space-x-[var(--space-2)]">
              <div className="flex border border-[var(--border-500)] rounded-[var(--radius-lg)] p-0.5 bg-[var(--bg-panel)]">
                <button 
                  onClick={() => setActivePreviewTab('all')} 
                  className={`px-[var(--space-3)] py-[var(--space-1)] text-body font-semibold rounded-[var(--radius-md)] transition-all ${activePreviewTab === 'all' ? 'bg-[var(--hover-600)] text-[var(--text-primary)]' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
                >
                  All ({totalItemsCount})
                </button>
                <button 
                  onClick={() => setActivePreviewTab('errors')} 
                  className={`px-[var(--space-3)] py-[var(--space-1)] text-body font-semibold rounded-[var(--radius-md)] transition-all ${activePreviewTab === 'errors' ? 'bg-[var(--color-error)]/20 text-[var(--color-error)]' : 'text-[var(--text-secondary)] hover:text-[var(--color-error)]'}`}
                >
                  Errors ({invalidRowsCount})
                </button>
                <button 
                  onClick={() => setActivePreviewTab('valid')} 
                  className={`px-[var(--space-3)] py-[var(--space-1)] text-body font-semibold rounded-[var(--radius-md)] transition-all ${activePreviewTab === 'valid' ? 'bg-[var(--color-success)]/20 text-[var(--color-success)]' : 'text-[var(--text-secondary)] hover:text-[var(--color-success)]'}`}
                >
              ...
              {isConverted && invalidRowsCount === 0 && (
                <>
                  <button 
                    ...
                    className="flex items-center space-x-[var(--space-1-5)] bg-[var(--color-success)] hover:bg-[var(--color-success)]/80 text-[var(--text-white)] font-bold text-body px-[var(--space-4)] py-[var(--space-1-5)] rounded-[var(--radius-lg)] shadow-[var(--shadow-elevation)] transition-all ml-[var(--space-4)]"
                  >
                  ...
                  <button 
                    ...
                    className="flex items-center space-x-[var(--space-1-5)] bg-[var(--color-brand)] hover:bg-[var(--color-brand-hover)] text-[var(--text-white)] font-bold text-body px-[var(--space-4)] py-[var(--space-1-5)] rounded-[var(--radius-lg)] shadow-[var(--shadow-elevation)] transition-all ml-[var(--space-2)]"
                  >
          ...
          {/* Core Table Grid */}
          <div className="flex-1 overflow-x-auto overflow-y-auto pr-2 min-h-0 border border-[var(--border-500)] rounded-[var(--radius-xl)] bg-[var(--bg-canvas)]">
            <table className="w-full text-left text-body whitespace-nowrap min-w-[1200px]">
              <thead className="bg-[var(--bg-surface)] text-[var(--text-secondary)] font-bold uppercase tracking-wider sticky top-0 z-20 border-b border-[var(--border-500)]">
                <tr>
                  <th className="px-[var(--space-4)] py-[var(--space-3)] text-center w-12">SR.NO</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)]">TOOL NO</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)]">DET NO</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)] text-center w-48">L × W × H</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)]">MATERIAL</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)] text-center w-20">QTY</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)] text-right">AP WT.</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)] text-right">TOTAL WT.</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)] text-right">RATE</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)] text-right">BASIC COST</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)] text-right">GST</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)] text-right">TOTAL</th>
                  <th className="px-[var(--space-4)] py-[var(--space-3)] text-center w-16">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--border-500)]">
                  ...
                  <tr 
                    ...
                    className={`transition-colors ${
                      row.validationError 
                        ...
                        : 'hover:bg-[var(--hover-600)]'
                    }`}
                  >
                    {/* SR.NO */}
                    <td className="px-[var(--space-4)] py-[var(--space-2-5)] text-center font-mono font-bold text-[var(--text-tertiary)]">
                      {idx + 1}
                    </td>

                    {/* TOOL NO */}
                    <td className="px-[var(--space-4)] py-[var(--space-2-5)] font-mono text-[var(--text-primary)]">
                      {parsedMetadata?.projectNumber || project.projectNumber}
                    </td>

                    {/* DET NO */}
                    <td className="px-[var(--space-4)] py-[var(--space-2-5)] font-mono font-bold text-[var(--text-primary)]">
                      {row.srNo}
                    </td>

                    {/* Raw size dimension fields (Editable: Length, Width, Height) */}
                    <td className="px-[var(--space-2)] py-[var(--space-2)] text-center">
                      <div className="flex items-center space-x-[var(--space-1-5)] justify-center">
                        <input 
                          type="text" 
                          placeholder="L"
                          value={row.length || ''} 
                          onChange={(e) => handleCellEdit(row.id, 'length', e.target.value)}
                          className="w-14 bg-[var(--bg-canvas)] border border-[var(--border-500)] rounded-[var(--radius-sm)] px-[var(--space-1-5)] py-[var(--space-1)] text-center font-mono text-[var(--text-primary)] text-body focus:outline-none focus:border-[var(--color-brand)]/50"
                        />
                        ...
                        <input 
                          ...
                          className="w-12 bg-[var(--bg-canvas)] border border-[var(--border-500)] rounded-[var(--radius-sm)] px-[var(--space-1-5)] py-[var(--space-1)] text-center font-mono text-[var(--text-primary)] text-body focus:outline-none focus:border-[var(--color-brand)]/50"
                        />
                      </div>
                    </td>
                    ...
                    <td className="px-[var(--space-2)] py-[var(--space-2)]">
                      <input 
                        ...
                        className="w-full bg-[var(--bg-canvas)] border border-[var(--border-500)] rounded-[var(--radius-sm)] px-[var(--space-2)] py-[var(--space-1)] text-body text-[var(--text-primary)] focus:outline-none focus:border-[var(--color-brand)]/50"
                      />
                    </td>
                    ...
                    <td className="px-[var(--space-2)] py-[var(--space-2)] text-center">
                      <input 
                        ...
                        className="w-16 bg-[var(--bg-canvas)] border border-[var(--border-500)] rounded-[var(--radius-sm)] px-[var(--space-2)] py-[var(--space-1)] text-center font-mono text-[var(--text-primary)] text-body focus:outline-none focus:border-[var(--color-brand)]/50"
                        min="1"
                      />
                    </td>

                    {/* AP WT. */}
                    <td className="px-[var(--space-4)] py-[var(--space-2-5)] text-right font-mono text-[var(--color-success)]">
                      {row.apWeight ? row.apWeight.toFixed(2) : '-'}
                    </td>

                    {/* TOTAL WT. */}
                    <td className="px-[var(--space-4)] py-[var(--space-2-5)] text-right font-mono text-[var(--color-success)] font-bold">
                      {row.totalWeight ? row.totalWeight.toFixed(2) : '-'}
                    </td>

                    {/* RATE */}
                    <td className="px-[var(--space-4)] py-[var(--space-2-5)] text-right font-mono text-[var(--text-primary)]">
                      {row.rate ? row.rate.toFixed(2) : '-'}
                    </td>

                    {/* BASIC */}
                    <td className="px-[var(--space-4)] py-[var(--space-2-5)] text-right font-mono text-[var(--color-info)] font-bold">
                      {row.basicCost ? row.basicCost.toFixed(2) : '-'}
                    </td>

                    {/* GST (Estimated 18%) */}
                    <td className="px-[var(--space-4)] py-[var(--space-2-5)] text-right font-mono text-[var(--text-tertiary)]">
                      {row.basicCost ? (row.basicCost * 0.18).toFixed(2) : '-'}
                    </td>

                    {/* TOTAL */}
                    <td className="px-[var(--space-4)] py-[var(--space-2-5)] text-right font-mono text-[var(--text-white)] font-bold">
                      {row.basicCost ? (row.basicCost * 1.18).toFixed(2) : '-'}
                    </td>

                    {/* Row Deletion Action */}
                    <td className="px-[var(--space-4)] py-[var(--space-2-5)] text-center">
                      <button 
                        onClick={() => handleDeleteRow(row.id)}
                        className="p-1 hover:bg-[var(--color-error)]/20 text-[var(--text-tertiary)] hover:text-[var(--color-error)] rounded-[var(--radius-sm)] transition-colors"
                        title="Delete Item"
                      >
          ...
          {validationRun && (invalidRowsCount > 0 || !parsedMetadata?.projectNumber) && (
            <div className="mt-[var(--space-4)] p-[var(--space-3)] bg-[var(--color-error)]/10 border border-[var(--color-error)]/20 rounded-[var(--radius-xl)] shrink-0">
              <h4 className="text-body font-bold text-[var(--color-error)] flex items-center uppercase tracking-wider mb-[var(--space-2)]">
                <AlertCircle className="w-4 h-4 mr-[var(--space-1-5)]" />
                Mapping Invariants / Core Failures
              </h4>
              <ul className="text-caption text-[var(--color-error)]/80 font-semibold space-y-[var(--space-1)] max-h-24 overflow-y-auto pr-2 custom-scrollbar">
                {!parsedMetadata?.projectNumber && (
                  <li className="flex items-start">
                    <span className="font-mono text-[var(--color-error)]/70 mr-[var(--space-2)] shrink-0">Global:</span>
                    <span>Project Number (Tool No) is missing from file headers.</span>
                  </li>
                )}
                {rows.filter(r => r.validationError !== null).map((r, i) => (
                  <li key={i} className="flex items-start">
                    <span className="font-mono text-[var(--color-error)]/70 mr-[var(--space-2)] shrink-0">Row {r.srNo}:</span>
