  ...
  return (
    <AppLayout>
      <div className="w-full flex-1 flex flex-col space-y-6">
        <PageHeader
          ...
          actions={
            <button 
              ...
              className="flex items-center px-4 py-2 bg-orange-600 hover:bg-orange-500 rounded-md text-sm font-semibold transition-colors"
            >
        ...
        {/* Tab Selection */}
        <div className="flex space-x-6 border-b border-hairline/60 mb-6">
          <button 
            onClick={() => setActiveTab('workforce')}
            className={`pb-3 text-sm font-semibold tracking-wider uppercase transition-colors relative ${activeTab === 'workforce' ? 'text-ink border-b-2 border-orange-500' : 'text-mute hover:text-ink'}`}
          >
            Workforce Directory
          </button>
          <button 
            onClick={() => setActiveTab('directory')}
            className={`pb-3 text-sm font-semibold tracking-wider uppercase transition-colors relative ${activeTab === 'directory' ? 'text-ink border-b-2 border-orange-500' : 'text-mute hover:text-ink'}`}
          >
        ...
        {loading ? (
          <div className="animate-pulse flex flex-col space-y-4">
            <div className="h-32 bg-black/5 rounded-md"></div>
            <div className="h-64 bg-black/5 rounded-md"></div>
          </div>
        ) : activeTab === 'workforce' ? (
      ...
      >
        <div className="space-y-4 text-ink">
          <div>
            <label className="block text-xs font-medium text-mute mb-1">Employee Code</label>
            <input 
              type="text" 
              value={newWorker.employeeCode}
              onChange={e => setNewWorker({...newWorker, employeeCode: e.target.value})}
              className="w-full bg-black/5 border border-hairline rounded-md px-3 py-2 text-ink text-sm focus:outline-none focus:border-emerald-500"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-mute mb-1">Type</label>
              <select 
                value={newWorker.employeeType}
                onChange={e => setNewWorker({...newWorker, employeeType: e.target.value})}
                className="w-full bg-black/5 border border-hairline rounded-md px-3 py-2 text-ink text-sm focus:outline-none focus:border-emerald-500 appearance-none"
              >
            ...
            <div>
              <label className="block text-xs font-medium text-mute mb-1">Hourly Rate (&#8377;)</label>
              <input 
                type="number" 
          ...
          <div>
            <label className="block text-xs font-medium text-mute mb-1">Designation</label>
            <input 
              type="text" 
          ...
          <div className="pt-6 flex justify-end space-x-3 border-t border-hairline mt-6">
            <button 
              type="button"
              onClick={() => setIsAddModalOpen(false)}
              className="px-4 py-2 rounded-md text-sm font-medium text-zinc-600 hover:text-ink hover:bg-black/5 transition-colors"
            >
              Cancel
            </button>
            <button 
              ...
              className="px-4 py-2 rounded-md text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] text-white transition-all shadow-[0_1px_2px_rgba(0,0,0,0.1),_inset_0_1px_0_rgba(255,255,255,0.2)] border border-emerald-500/30 cursor-pointer"
            >
              Add Worker
            </button>
          </div>
...
function WorkerCard({ worker, isExternal = false }: { worker: any, isExternal?: boolean }) {
  ...
  return (
    <div className={`p-5 rounded-md border ${isExternal ? 'bg-purple-900/10 border-purple-500/20' : 'bg-black/5 border-hairline'} shadow-sm relative overflow-hidden group`}>
      <div className="absolute top-0 right-0 p-4">
        <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded-md ${isExternal ? 'bg-purple-500/20 text-purple-300' : 'bg-blue-500/20 text-blue-300'}`}>
          {worker.employeeType}
        </span>
      </div>
      
      <div className="flex items-center space-x-4 mb-4">
        <div className="w-12 h-12 rounded-md bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center text-ink font-semibold text-lg border border-hairline">
          {worker.name.substring(0, 2).toUpperCase()}
        </div>
        <div>
          <h3 className="font-semibold text-ink text-lg">{worker.name}</h3>
          <p className="text-sm text-mute">{worker.designation || 'Worker'}</p>
        </div>
      </div>
          ...
          {editingRate ? (
            <div className="flex items-center space-x-2">
              ...
              <input 
                ...
                className="w-20 bg-black/5 border border-black/20 rounded px-2 py-1 text-ink text-sm"
              />
              <button onClick={handleUpdateRate} className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded">Save</button>
              <button onClick={() => setEditingRate(false)} className="text-xs text-mute hover:text-ink">Cancel</button>
            </div>
          ) : (
            <div className="flex items-center space-x-2">
              <p className="font-semibold text-ink">&#8377;{worker.hourlyRate} <span className="text-slate-500 font-normal text-sm">/ hr</span></p>
              <button onClick={() => setEditingRate(true)} className="p-1 hover:bg-black/10 rounded-md transition-colors">
                <Edit2 className="w-3 h-3 text-mute" />
              </button>
            </div>
          )}
        </div>
...
function HourlyRatesDirectoryTable({ employees, machines, users, onReload }: { employees: any[], machines: any[], users: any[], onReload: () => void }) {
  ...
  return (
    <div className="bg-canvas/50 border border-hairline rounded-md p-6 relative overflow-hidden shadow-level-4 backdrop-blur-2xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 rounded-md bg-orange-500/10 text-orange-400 flex items-center justify-center border border-orange-500/20">
            <DollarSign className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-lg font-semibold text-ink tracking-tight">Consolidated Hourly Rates</h3>
            <p className="text-xs text-mute">View and update billing rates for all personnel, machines, and administrative owners.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input 
              ...
              className="w-full bg-black/5 border border-hairline rounded-md pl-9 pr-4 py-2 text-ink text-xs focus:outline-none focus:border-orange-500 transition-colors"
            />
          </div>
          <select
            value={filterCategory}
            onChange={e => setFilterCategory(e.target.value)}
            className="bg-canvas border border-hairline rounded-md px-3 py-2 text-ink text-xs focus:outline-none focus:border-orange-500"
          >
      ...
      <div className="overflow-x-auto rounded-md border border-hairline/60">
        <table className="w-full text-left text-xs whitespace-nowrap">
          <thead className="bg-black/5 text-mute border-b border-hairline/60 font-semibold uppercase tracking-wider text-[10px]">
            <tr>
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={item.id} className="hover:bg-black/[0.02] transition-colors group">
                  <td className="px-6 py-4 font-mono font-semibold text-mute">{item.code}</td>
                  ...
                  <td className="px-6 py-4 text-mute font-medium">{item.type}</td>
                  <td className="px-6 py-4 text-mute">{item.designation}</td>
                  <td className="px-6 py-4">
                    {isEditing ? (
                      <div className="flex items-center space-x-2">
                        ...
                        <input 
                          ...
                          className="w-20 bg-black/5 border border-black/20 rounded px-2 py-1 text-ink text-xs font-semibold font-mono focus:outline-none focus:border-orange-500"
                        />
                      </div>
                    ) : (
                      <span className="font-mono font-semibold text-ink text-xs">â‚¹{item.rate.toFixed(2)}</span>
                    )}
                  </td>
                    ...
                    {isEditing ? (
                      <div className="flex items-center justify-end space-x-2">
                        <button 
                          ...
                          disabled={saving}
                          className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded font-semibold hover:bg-emerald-500/30 transition-colors disabled:opacity-50"
                        >
                          {saving ? 'Saving...' : 'Save'}
                        </button>
                        <button 
                      ...
                      >
                        <Edit2 className="w-3.5 h-3.5 text-mute group-hover:text-ink" />
                      </button>
                    )}
                  </td>
                </tr>
