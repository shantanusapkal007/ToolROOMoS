    ...
    <div className="flex h-screen bg-transparent relative w-full overflow-hidden">
      ...
      <div className="flex-1 h-full flex flex-col relative z-0 pl-[5.5rem] pr-8 animate-fade-in py-8 overflow-y-auto hide-scrollbar">
        ...
        <div className="mt-8 flex justify-between items-center mb-6">
          <div className="relative w-80">
            ...
            <input 
              ...
              className="w-full bg-black/5 border border-black/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-zinc-900 focus:outline-none focus:border-amber-500/50 transition-colors"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
          <button className="bg-black/5 hover:bg-black/10 border border-black/10 px-4 py-2.5 rounded-xl text-sm font-bold text-zinc-900 uppercase tracking-wider flex items-center transition-colors">
        ...
        {isLoading ? (
          ...
          <div className="glass-panel border border-black/10 rounded-2xl overflow-hidden relative">
             ...
             <table className="w-full text-left border-collapse relative z-10">
               <thead>
                 <tr className="border-b border-black/10 bg-black/[0.02]">
                   <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">Material / Batch</th>
                   <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">Warehouse</th>
                   <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider text-right">Available Qty</th>
                   <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider text-right">Unit Value</th>
                   <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider text-right">Total Value</th>
               ...
               <tbody>
                 {filteredLedger?.map((batch: any) => {
                   ...
                   return (
                     <tr key={batch.id} className="border-b border-black/5 hover:bg-black/[0.02] transition-colors group">
                       <td className="p-4">
                         <div className="flex items-center">
                           ...
                           <div>
                             <div className="font-bold text-zinc-900 text-sm">{batch.material?.materialCode} <span className="text-zinc-500 font-normal">· {batch.material?.materialGrade}</span></div>
                       ...
                       <td className="p-4">
                         <div className="text-sm font-medium text-zinc-600">{batch.location?.warehouse?.warehouseName || '—'}</div>
                       ...
                       <td className="p-4 text-right">
                         <div className="flex items-center justify-end space-x-2">
                           ...
                           <span className="font-mono text-zinc-900 font-bold">{batch.currentQty}</span>
                       ...
                       <td className="p-4">
                         <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${
                           ...
                           'bg-slate-500/10 text-zinc-500 border border-slate-500/20'
                         }`}>
                           {batch.status}
                         </span>
                       </td>
