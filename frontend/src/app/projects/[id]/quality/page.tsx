      ...
      <div className="flex items-center p-1 bg-zinc-100 rounded-[12px] border border-border-gray/80 overflow-x-auto gap-1">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-[12px] text-xs font-semibold transition-all cursor-pointer whitespace-nowrap ${
            activeTab === "REGISTERS" ? "bg-white text-ink shadow-subtle" : "text-mute hover:text-ink"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-[12px] text-xs font-semibold transition-all cursor-pointer whitespace-nowrap ${
            activeTab === "CMM_DIMENSIONS" ? "bg-white text-ink shadow-subtle" : "text-mute hover:text-ink"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-[12px] text-xs font-semibold transition-all cursor-pointer whitespace-nowrap ${
            activeTab === "NCR_REWORK" ? "bg-white text-ink shadow-subtle" : "text-mute hover:text-ink"
          }`}
        >
        ...
        <button
          onClick={() => setActiveTab("QC_CERTIFICATE")}
          className={`flex items-center gap-2 px-4 py-2 rounded-[12px] text-xs font-semibold transition-all cursor-pointer whitespace-nowrap ${
            activeTab === "QC_CERTIFICATE" ? "bg-white text-ink shadow-subtle" : "text-mute hover:text-ink"
          }`}
        >
          <BadgeCheck className="w-4 h-4 text-emerald-600" />
            ...
            <div className="flex items-center gap-1 bg-zinc-100 p-1 rounded-[12px] border border-border-gray">
              {["ALL", "INCOMING", "IN_PROCESS", "FINAL_PDI"].map((f) => (
                <button
                  ...
                  className={`px-3 py-1 text-[11px] font-semibold rounded-[12px] transition-colors cursor-pointer ${
                    selectedInspectionFilter === f ? "bg-white text-ink shadow-subtle" : "text-mute hover:text-ink"
                  }`}
                >
                  {f.replace(/_/g, " ")}
                </button>
      ...
      {activeTab === "QC_CERTIFICATE" && (
        <div className="space-y-4">
          ...
          <div className="bg-white p-8 rounded-[12px] border border-border-gray shadow-subtle space-y-6 text-ink font-sans max-w-4xl mx-auto print:border-none print:shadow-none">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-canvas rounded-[12px] border border-border-gray text-xs font-sans">
              <div>
                <span className="text-mute text-[10px] uppercase font-semibold">Project Number</span>
                <div className="font-semibold text-ink">{project?.projectNumber || "KTD-2026-001"}</div>
              </div>
              <div>
                <span className="text-mute text-[10px] uppercase font-semibold">Tool / Part Name</span>
                <div className="font-semibold text-ink">{project?.partName || "Main Press Tooling Assembly"}</div>
              </div>
              <div>
                <span className="text-mute text-[10px] uppercase font-semibold">Customer Name</span>
                <div className="font-semibold text-ink">{project?.customer?.companyName || "Internal Manufacturing"}</div>
              </div>
              <div>
                <span className="text-mute text-[10px] uppercase font-semibold">Inspection Date</span>
                <div className="font-semibold text-ink">{new Date().toLocaleDateString("en-GB")}</div>
              </div>
            </div>
