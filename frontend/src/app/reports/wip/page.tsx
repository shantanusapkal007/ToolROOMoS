    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in zoom-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-mute mt-2 font-mono text-sm">Real-time shop floor capital tracking</p>
        </div>
      </div>

      {/* KPI Row */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-black/5 backdrop-blur-xl border border-border-gray rounded-[12px] p-6 relative overflow-hidden group hover:border-cyan-500/30 transition-all duration-300">
          ...
          <p className="text-mute text-xs font-semibold uppercase tracking-wider mb-2">Total Capital in WIP</p>
          <p className="text-4xl font-semibold text-ink">{formatCurrency(currentValuation)}</p>
        </div>
        <div className="bg-black/5 backdrop-blur-xl border border-border-gray rounded-[12px] p-6 relative overflow-hidden group hover:border-emerald-500/30 transition-all duration-300">
          ...
          <p className="text-mute text-xs font-semibold uppercase tracking-wider mb-2">Accrued Material Cost</p>
          <p className="text-4xl font-semibold text-ink">{formatCurrency(materialCost)}</p>
        </div>
        <div className="bg-black/5 backdrop-blur-xl border border-border-gray rounded-[12px] p-6 relative overflow-hidden group hover:border-orange-500/30 transition-all duration-300">
          ...
          <p className="text-mute text-xs font-semibold uppercase tracking-wider mb-2">Accrued Machine Value</p>
          <p className="text-4xl font-semibold text-ink">{formatCurrency(machineCost)}</p>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-border-gray/60 bg-black/5">
                ...
                <th className="p-4 text-xs font-semibold text-mute uppercase tracking-wider">Material</th>
                <th className="p-4 text-xs font-semibold text-mute uppercase tracking-wider">Current Location</th>
                <th className="p-4 text-xs font-semibold text-mute uppercase tracking-wider text-right">Qty</th>
                <th className="p-4 text-xs font-semibold text-mute uppercase tracking-wider text-right">Accrued Value</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {ledger.map((row: any) => (
                <tr key={row.id} className="hover:bg-black/5 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="font-semibold text-zinc-700">{row.material?.materialCode}</div>
                    <div className="text-mute text-xs truncate max-w-[200px]">{row.material?.materialName}</div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-right font-mono text-ink">
