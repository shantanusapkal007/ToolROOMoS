    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in zoom-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-2 font-mono text-sm">Real-time shop floor capital tracking</p>
        </div>
      </div>

      {/* KPI Row */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 relative overflow-hidden group hover:border-cyan-500/30 transition-all duration-300">
          ...
          <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2">Total Capital in WIP</p>
          <p className="text-4xl font-black text-white">{formatCurrency(currentValuation)}</p>
        </div>
        <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 relative overflow-hidden group hover:border-emerald-500/30 transition-all duration-300">
          ...
          <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2">Accrued Material Cost</p>
          <p className="text-4xl font-black text-white">{formatCurrency(materialCost)}</p>
        </div>
        <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 relative overflow-hidden group hover:border-orange-500/30 transition-all duration-300">
          ...
          <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2">Accrued Machine Value</p>
          <p className="text-4xl font-black text-white">{formatCurrency(machineCost)}</p>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-white/5 bg-white/5">
                ...
                <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Material</th>
                <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Current Location</th>
                <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-right">Qty</th>
                <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-right">Accrued Value</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {ledger.map((row: any) => (
                <tr key={row.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="font-bold text-slate-200">{row.material?.materialCode}</div>
                    <div className="text-slate-400 text-xs truncate max-w-[200px]">{row.material?.materialName}</div>
                  </td>
                  <td className="p-4">
