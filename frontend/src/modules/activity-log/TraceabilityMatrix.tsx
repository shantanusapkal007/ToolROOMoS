...
export function TraceabilityMatrix() {
  ...
  return (
    <div className="w-full h-full flex flex-col min-h-0 space-y-4">
      ...
      <div className="flex items-center justify-between mb-6 flex-shrink-0">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-display font-semibold tracking-tight text-ink">Total System Traceability</h1>
        ...
        <button 
          onClick={loadTabData}
          className="flex items-center gap-2 px-4 py-2.5 bg-white/70 backdrop-blur-md border border-white/60 shadow-sm hover:shadow-subtle transition-all rounded-[12px] text-sm font-medium text-zinc-700"
        >
      ...
      <div className="flex items-center gap-2 mb-6 p-1.5 bg-zinc-900/5 backdrop-blur-xl border border-white/40 rounded-[12px] shadow-inner flex-shrink-0">
        {[
          ...
          return (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2.5 py-3 px-4 rounded-[12px] font-medium text-sm transition-all relative ${
                isActive 
                  ? 'bg-white text-ink shadow-elevation font-semibold border border-border-gray/60' 
                  : 'text-zinc-600 hover:text-ink hover:bg-white/40'
              }`}
            >
              {tab.icon}
              <span>{tab.label}</span>
              {tab.count !== undefined && (
                <span className={`px-2 py-0.5 text-xs rounded-full ${isActive ? 'bg-zinc-100 text-zinc-800 font-semibold' : 'bg-zinc-200/60 text-zinc-600'}`}>
                  {tab.count}
                </span>
              )}
            </button>
      ...
      {activeTab === 'AUDIT' && (
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 flex-shrink-0">
            {[
              ...
              <div key={idx} className="glass-panel p-4 flex items-center justify-between border border-white/60">
                <div>
                  <p className="text-xs font-medium text-mute">{stat.title}</p>
                  <h3 className="text-xl font-semibold text-ink mt-1">{stat.value}</h3>
                </div>
                <div className="p-2.5 bg-white/80 rounded-[12px] shadow-sm border border-border-gray/60">
                  {stat.icon}
                </div>
              </div>
            ))}
          ...
          {/* Filters */}
          <div className="glass p-4 rounded-[12px] flex items-center gap-4 mb-4 flex-shrink-0 border border-white/60 shadow-sm">
            <div className="flex-1 relative">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2 bg-white/60 border border-border-gray rounded-[12px] text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
              />
            </div>
            <select 
              value={auditEntity}
              onChange={(e) => setAuditEntity(e.target.value)}
              className="px-4 py-2 bg-white/60 border border-border-gray rounded-[12px] text-sm font-medium text-zinc-700 outline-none"
            >
          ...
          {/* Audit List */}
          <div className="flex-1 overflow-y-auto custom-scrollbar bg-white/40 rounded-[12px] border border-white/60 p-6 shadow-inner">
            {logs.length > 0 ? (
              <HistoryTimeline events={timelineEvents} />
            ) : (
              <div className="h-full flex flex-col items-center justify-center text-mute py-12">
      ...
      {activeTab === 'TASKS' && (
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          {/* Controls */}
          <div className="glass p-4 rounded-[12px] flex items-center gap-4 mb-4 flex-shrink-0 border border-white/60">
            <div className="flex-1 relative">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400" />
              <input 
                type="text" 
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className={`px-2.5 py-1 text-xs font-semibold rounded-[12px] border ${
                      task.category === 'PROJECT_TASK' 
                        ? 'bg-primary-subtle0/10 text-primary-dark border-blue-500/20' 
                  ...
                  <h3 className="font-semibold text-ink text-base mb-1">{task.taskName}</h3>
                  <p className="text-xs text-mute mb-4 line-clamp-2">{task.description}</p>

                  {/* Assigner -> Assignee Pipeline */}
                  <div className="p-3 bg-zinc-900/5 rounded-[12px] border border-border-gray/60 space-y-2 mb-4">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-border-gray/60 text-xs">
                  <span className="text-mute font-mono">
                    {task.projectNumber || 'Global Asset'}
                  </span>
                  <span className={`px-2.5 py-1 rounded-full font-semibold ${
                    task.status === 'COMPLETED' ? 'bg-emerald-500/10 text-emerald-700' : 'bg-primary-subtle0/10 text-primary-dark'
                  }`}>
                    {task.status}
                  </span>
                </div>
          ...
          <div className="glass p-5 rounded-[12px] flex items-center gap-4 mb-6 flex-shrink-0 border border-white/70 shadow-sm">
            <div className="flex-1 relative">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white/70 border border-border-gray rounded-[12px] text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
              />
            </div>
            <button
              onClick={handleGenealogySearch}
              className="px-5 py-2.5 bg-zinc-900 text-white rounded-[12px] text-sm font-medium shadow-elevation hover:bg-zinc-800 transition-colors"
            >
              Trace Pedigree
            </button>
          </div>
            ...
            {genealogyData.length > 0 ? (
              genealogyData.map((item, idx) => (
                <div key={idx} className="glass-panel p-6 border border-white/80 shadow-elevation">
                  ...
                  <div className="flex items-start justify-between pb-6 border-b border-border-gray mb-6">
                    <div>
                      <div className="flex items-center gap-3">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-semibold text-emerald-600">{item.currentQty} / {item.receivedQty} KG</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
                    ...
                    <div className="p-4 bg-white/60 rounded-[12px] border border-border-gray/60 relative">
                      <div className="flex items-center gap-2 mb-3 text-primary-dark font-semibold text-sm">
                    ...
                    <div className="p-4 bg-white/60 rounded-[12px] border border-border-gray/60 relative">
                      ...
                      {item.issuances.length > 0 ? (
                        item.issuances.map((iss, iIdx) => (
                          <div key={iIdx} className="text-xs space-y-1 text-zinc-600 border-b border-border-gray/60 pb-2 last:border-0">
                            <p><strong>Issue #:</strong> {iss.issueNumber}</p>
                            <p><strong>Issued Qty:</strong> {iss.issuedQty} KG</p>
                            <p><strong>Target Shop:</strong> <span className="font-semibold text-primary-dark">{getProductionShopDetails(iss.productionSection).label}</span></p>
                    ...
                    <div className="p-4 bg-white/60 rounded-[12px] border border-border-gray/60 relative">
                      ...
                      {item.machineLogs.length > 0 ? (
                        item.machineLogs.map((mLog, mIdx) => (
                          <div key={mIdx} className="text-xs space-y-1 text-zinc-600 border-b border-border-gray/60 pb-2 last:border-0">
      ...
      {activeTab === 'ASSETS' && (
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          {/* Asset List */}
          <div className="flex-1 overflow-y-auto custom-scrollbar bg-white/40 rounded-[12px] border border-white/60 p-4 shadow-inner">
            <table className="w-full text-left text-xs text-zinc-700">
              <thead className="bg-zinc-900/5 text-mute uppercase font-semibold text-[10px] tracking-wider border-b border-border-gray">
                <tr>
              ...
              <tbody className="divide-y divide-black/5 font-medium">
                {assets.map((asset) => (
                  <tr key={asset.id} className="hover:bg-white/60 transition-colors">
                    <td className="px-4 py-3 font-mono font-semibold text-ink">{asset.issueNumber}</td>
                    <td className="px-4 py-3 font-semibold text-primary-dark">{asset.assetCode} — {asset.assetName}</td>
                    <td className="px-4 py-3">{asset.issuedByName}</td>
                    <td className="px-4 py-3 font-semibold text-zinc-800">{asset.employeeName}</td>
