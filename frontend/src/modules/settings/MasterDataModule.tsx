  ...
  return (
    <div className="flex-1 h-full flex flex-col relative z-0 pl-32 pr-12 animate-fade-in py-12">
      ...
      <div className="flex flex-1 overflow-hidden mt-4 gap-8">
        ...
        {/* Premium Sidebar Nav */}
        <div className="w-80 shrink-0 flex flex-col space-y-3 overflow-y-auto pr-4 hide-scrollbar pb-20">
          {navigation.map((item, idx) => {
            ...
            return (
              <button
                ...
                className={`relative group flex items-center w-full p-4 rounded-2xl transition-all duration-500 text-left overflow-hidden ${
                  isActive 
                    ? 'bg-[#0B1018]/80 border-l-4 border-l-emerald-500 border-y border-r border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)]' 
                    : 'bg-transparent border border-transparent hover:bg-white/5 hover:border-white/10'
                }`}
                style={{ animation: `slideUp 0.4s ease-out ${idx * 0.05}s forwards`, opacity: mounted ? 1 : 0 }}
              >
                {/* Active Glow */}
                {isActive && <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/10 to-transparent pointer-events-none"></div>}
                ...
                <div className={`flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-500 relative z-10 ${
                  isActive 
                    ? 'bg-emerald-500/20 text-emerald-400 shadow-[inset_0_0_15px_rgba(16,185,129,0.3)]' 
                ...
                <div className={`transition-all duration-500 absolute right-4 ${isActive ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4 group-hover:translate-x-0 group-hover:opacity-50'}`}>
                  <ChevronRight className={`w-5 h-5 ${isActive ? 'text-emerald-400' : 'text-slate-400'}`} />
                </div>
              </button>
            );
          })}
